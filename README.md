...
**Faithfulness** — the answer is decomposed into atomic claims (sentence-split). Each claim is embedded and compared to every sentence in the retrieved contexts. `score = supported_claims / total_claims`. A claim is supported if its max cosine similarity to any context sentence exceeds the threshold (default 0.7). Optional LLM-as-judge path for higher accuracy.
